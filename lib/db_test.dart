...
void main() async {
  ...
  // Future<void> createNewTableIfNotExist() async {
  ...
  Future<List<Dog>> dogsByName(String sName) async {
    // Get a reference to the database.
    final db = await database;

    // Query the table for all The Dogs.
    final List<Map<String, dynamic>> maps =
        await db.rawQuery("Select * from $TABLE_NAME where name = '$sName'");
  ...
  int fidoId = await insertDog(fido);
  int oscarId = await insertDog(oscar);
  ...
  print(await dogs2()); // Prints Fido with age 42.

  // Delete Fido from the database.
  //await deleteDog(fido.id);

  // Print the list of dogs (empty).
  print(await dogsByName('Oscar'));
}
